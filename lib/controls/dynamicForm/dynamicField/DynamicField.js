...
var DynamicFieldBase = /** @class */ (function (_super) {
    ...
    function DynamicFieldBase(props) {
        ...
        _this.getNumberErrorText = function () {
            ...
            if (fieldType === "Currency" && cultureName) {
                ...
                if (minValue)
                    minValueCur = Intl.NumberFormat(cultureName, { style: 'currency', currency: CurrencyMap[countryCode] }).format(minValue); // eslint-disable-line @typescript-eslint/no-explicit-any
                if (maxValue)
                    maxValueCur = Intl.NumberFormat(cultureName, { style: 'currency', currency: CurrencyMap[countryCode] }).format(maxValue); // eslint-disable-line @typescript-eslint/no-explicit-any
            }
        ...
        _this.MultiChoice_selection = function (event, item) {
            var changedValue = _this.state.changedValue;
            try {
                var selectedItemArr_1; // eslint-disable-line @typescript-eslint/no-explicit-any
    ...
    DynamicFieldBase.prototype.componentDidUpdate = function () {
